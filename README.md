...
Так как нельзя было использовать БД, я попробовал реализовать собстенный in-memory storage
 для хранения данных в рантайме приложения. В основном моя, так называемая "БД"
 состоит из map, вложенных друг в друга. Так же для реализации поиска записей
 я использовал префиксное дерево, потому что счел его вполне оптимальным решением для
 моей задачи.

### Example Request
`POST /api/user` - создает пользователя(user)
...
### Example Request
`GET /api/user/19cfe3f9-8bc0-41cc-8dba-27b37efd9b74 - получение одного пользователя по id
...
### Example Request
`DELETE /api/user` - удаление пользователя по id
